...
PYTHON_MODULE_BEGIN( gpuseg )
    ...
    PYTHON_CLASS_BEGIN( "VolumeFileDesc", VolumeFileDesc )
        ...
        PYTHON_CLASS_FIELD_READWRITE( "leftFileName",     VolumeFileDesc::leftFileName )
        PYTHON_CLASS_FIELD_READWRITE( "rightFileName",    VolumeFileDesc::rightFileName )
...
VolumeDesc Engine::LoadVolume( const VolumeFileDesc& volumeFileDesc, const core::String& parameterFile )
{
    ...
    mGPUSegRenderStrategy->SetLeftSourceVolumeTexture( mSegmenter->GetLeftSourceVolumeTexture() );
    mGPUSegRenderStrategy->SetRightSourceVolumeTexture( mSegmenter->GetRightSourceVolumeTexture() );
    ...
    mGPUSegRenderStrategy->SetConstraintVolumeTexture( mSegmenter->GetConstraintVolumeTexture() );
...
void Engine::SaveSegmentationAs( const core::String& fileName )
{
    ...
    delete [] segmentationVolume.leftVolumeData;
}
...
void Engine::BeginPlaceSeed( SeedType seedType )
{
    if ( !sPlaceSeedBegun )
    {
        ...
        mGPUSegRenderStrategy->BeginPlaceSeed( seedType );
    }
}

void Engine::PlaceSeed( int screenX, int screenY, SeedType seedType )
{
    if ( sPlaceSeedBegun )
    {
        ...
        mGPUSegRenderStrategy->PlaceSeed( virtualScreenX, virtualScreenY, seedType );
        mFinishedSegmentation = false;
#ifdef PRINT_USER_INTERACTION_TIME
        rendering::TextConsole::PrintToStaticConsole( "userInteractionTime", "" );
#endif
...
void Engine::EndPlaceSeed( SeedType seedType )
{
    if ( sPlaceSeedBegun )
    {
        mGPUSegRenderStrategy->EndPlaceSeed( seedType );
...
void Engine::BeginInitializeSeed( SeedType seedType )
{
    if ( mVolumeLoaded )
    {
        if ( seedType == SeedType_Foreground )
        {
            mSegmenter->BeginInitializeSeed();
        }
        else
        if ( seedType == SeedType_Background )
        {
            mSegmenter->BeginInitializeBackgroundConstraint();
        }
...
void Engine::InitializeSeed( const math::Vector3& seedCoordinates, unsigned int sphereSize, SeedType seedType )
{
    if ( mVolumeLoaded )
    {
        if ( seedType == SeedType_Foreground )
        ...
        if ( seedType == SeedType_Background )
        {
            mSegmenter->InitializeBackgroundConstraint( seedCoordinates, sphereSize );
        }
    }
}

void Engine::EndInitializeSeed( SeedType seedType )
{
    if ( mVolumeLoaded )
    {
        if ( seedType == SeedType_Foreground )
        {
            mSegmenter->EndInitializeSeed();
        }
        else
        if ( seedType == SeedType_Background )
        {
            mSegmenter->EndInitializeBackgroundConstraint();
        }
    }
}
