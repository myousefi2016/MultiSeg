...
static rendering::rtgi::Texture*           sLeftSourceVolumeTexture          = NULL;
static rendering::rtgi::Texture*           sRightSourceVolumeTexture         = NULL;
...
static rendering::rtgi::Texture*           sConstraintVolumeTexture          = NULL;
...
static content::Parameter< float > sRenderLeftOrRightSourceVolume( "GPUSegRenderStrategy", "renderLeftOrRightSourceVolume" );
...
void GPUSegRenderStrategy::SetLeftSourceVolumeTexture( rendering::rtgi::Texture* texture )
{
    AssignRef( sLeftSourceVolumeTexture, texture );
}

void GPUSegRenderStrategy::SetRightSourceVolumeTexture( rendering::rtgi::Texture* texture )
{
    AssignRef( sRightSourceVolumeTexture, texture );
}
...
void GPUSegRenderStrategy::SetConstraintVolumeTexture( rendering::rtgi::Texture* texture )
{
    AssignRef( sConstraintVolumeTexture, texture );
}
...
void GPUSegRenderStrategy::UnloadVolume()
{
    ...
    AssignRef( sLeftSourceVolumeTexture,      NULL );
    AssignRef( sRightSourceVolumeTexture,     NULL );
...
void GPUSegRenderStrategy::Render( content::Ref< rendering::Scene > scene, content::Ref< rendering::Camera > camera )
{
    if ( sVolumeLoaded )
    {
        ...
        float                     currentSliceLowerBound     = floor( sCuttingPlaneZValue.GetValue() * volumeDesc.numVoxelsZ );
        float                     currentCuttingPlaneZValue  = ( ( currentSliceLowerBound + 0.5f ) / volumeDesc.numVoxelsZ );
        ...
        if ( sOriginalVolumeDesc.upDirection == math::Vector3(  0,  0, -1 ) )
        {
            ...
            currentBoundingBoxExtent[ math::Z ] =
                - boundingBoxHalfDimensions[ math::Z ] + ( boundingBoxHalfDimensions[ math::Z ] * math::Clamp( 0, 1, currentCuttingPlaneZValue ) * 2 );
        ...
        rendering::rtgi::Texture* currentSourceVolumeTexture = sRenderLeftOrRightSourceVolume.GetValue() < 0.5f ? sLeftSourceVolumeTexture : sRightSourceVolumeTexture;

        sVolumeShader->SetShaderParameter( "sourceVolumeSampler",                   currentSourceVolumeTexture,             textureSamplerStateDesc );
        ...
        sVolumeShader->SetShaderParameter( "constraintVolumeSampler",               sConstraintVolumeTexture,               textureSamplerStateDesc );
        ...
        sVolumeShader->SetShaderParameter( "cuttingPlaneZValue",                    currentCuttingPlaneZValue );
...
void GPUSegRenderStrategy::BeginPlaceSeed( SeedType seedType )
{
    sCurrentlySketching = true;
    sEngine->BeginInitializeSeed( seedType ); 
}

void GPUSegRenderStrategy::PlaceSeed( int virtualScreenX, int virtualScreenY, SeedType seedType )
{
         ...
         intersection[ math::Y ] >= - boundingBoxHalfDimensions[ math::Y ] && intersection[ math::Y ] <= boundingBoxHalfDimensions[ math::Y ]  )
    {
        ...
        float      currentSliceLowerBound     = floor( sCuttingPlaneZValue.GetValue() * paddedVolumeDesc.numVoxelsZ );
        float      currentCuttingPlaneZValue  = ( ( currentSliceLowerBound + 0.5f ) / paddedVolumeDesc.numVoxelsZ );
        ...
        math::Vector3 intersectionPaddedBoundingBoxSpace        = intersection + paddedBoundingBoxHalfDimensionsWorldSpace;
        math::Vector3 intersectionNonPaddedTextureSpace         = math::Vector3(
                                                                      ...
                                                                      currentCuttingPlaneZValue * sOriginalVolumeDesc.numVoxelsZ );

        sEngine->InitializeSeed( intersectionNonPaddedTextureSpace, 2.0f, seedType ); 
    }                                                                       
}                                                                           

void GPUSegRenderStrategy::EndPlaceSeed( SeedType seedType )
{
    //if ( sSketchPoints.Size() > 2 )
    //{
    //    math::Vector3 startIntersection = ComputeIntersectionWithCuttingPlane( sSketchPoints.At( 0 ) );
    //    math::Vector3 endIntersection   = ComputeIntersectionWithCuttingPlane( sSke  tchPoints.At( sSketchPoints.Size() - 1 ) );
    ...
    sEngine->EndInitializeSeed( seedType ); 
