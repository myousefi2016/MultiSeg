...
{
    boost::filesystem::path leftFilePath( volumeFileDesc.leftFileName.ToStdString() );
    boost::filesystem::path rightFilePath( volumeFileDesc.rightFileName.ToStdString() );

    ReleaseAssert( boost::filesystem::exists( leftFilePath ) );
    ReleaseAssert( boost::filesystem::exists( rightFilePath ) );

    if ( boost::filesystem::is_directory( leftFilePath ) )
    {
        ReleaseAssert( boost::filesystem::is_directory( rightFilePath ) );

        VolumeDesc leftVolumeDesc  = LoadVolumeDicom( leftFilePath,  volumeFileDesc );
        VolumeDesc rightVolumeDesc = LoadVolumeDicom( rightFilePath, volumeFileDesc );

        leftVolumeDesc.rightVolumeData = rightVolumeDesc.leftVolumeData;

        return leftVolumeDesc;
    }
    else
    {
        ReleaseAssert( !boost::filesystem::is_directory( rightFilePath ) );

        VolumeDesc leftVolumeDesc  = LoadVolumeRaw( leftFilePath,  volumeFileDesc );
        VolumeDesc rightVolumeDesc = LoadVolumeRaw( rightFilePath, volumeFileDesc );
...
{
    ReleaseAssert( volumeDesc.leftVolumeData  != NULL );
    ReleaseAssert( volumeDesc.rightVolumeData != NULL );
...
VolumeDesc VolumeLoader::LoadVolumeDicom( const boost::filesystem::path& dicomPath, const VolumeFileDesc& volumeFileDesc )
{
    ...
    volumeDesc.leftVolumeData         = volumeData;
    volumeDesc.rightVolumeData        = NULL;
...
VolumeDesc VolumeLoader::LoadVolumeRaw( const boost::filesystem::path& rawPath, const VolumeFileDesc& volumeFileDesc )
{
    ...
    errno_t errorCode;
    errorCode = fopen_s( &filePointer, rawPath.native_file_string().c_str(), "rb" );
