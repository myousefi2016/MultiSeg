...
void VolumeSaver::SaveVolumeAsDicom( const boost::filesystem::path &dicomPath, const VolumeDesc &volumeDesc )
{
    ...
    unsigned int   bitsAllocatedStored  = volumeDesc.numBytesPerVoxel * 8;
    ...
    for( unsigned int z = 0; z < volumeDesc.numVoxelsZ; z++ )
    {
        ...
        dcmDataset->putAndInsertString( DCM_SourceApplicationEntityTitle, "MULTISEG" );
        ...
        dcmDataset->putAndInsertUint16( DCM_BitsAllocated,           bitsAllocatedStored );
        dcmDataset->putAndInsertUint16( DCM_BitsStored,              bitsAllocatedStored );
        dcmDataset->putAndInsertUint16( DCM_HighBit,                 bitsAllocatedStored - 1 );
        ...
        dcmDataset->putAndInsertUint8Array( DCM_PixelData, &( reinterpret_cast< unsigned char* >( volumeDesc.leftVolumeData )[ z * sliceNumBytes ] ), sliceNumBytes );
