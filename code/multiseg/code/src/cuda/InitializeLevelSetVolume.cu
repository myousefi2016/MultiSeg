                                           ...
                                           float                outOfPlaneAnisotropy )
{
    ...
    bool       writeConstraintValue = false;
    
    if ( distance < sphereSize && elementCoordinates.z == seedRegion.z )
    {
        writeConstraintValue = true;
    ...
    if ( writeConstraintValue )
    {
        deviceConstraintData[ arrayIndexUntiled ] = 127;
    }
...
__global__ void  InitializeConstraintVolumeKernel( char*                deviceConstraintData,
                                                   dim3                 volumeDimensions,
                                                   int                  logicalNumThreadBlocksX,
                                                   float                invNumThreadBlocksX,
                                                   dim3                 seedRegion,
...
extern "C" void CudaInitializeConstraintVolume( char*                   deviceConstraintData,
                                                dim3                    volumeDimensions,
                                                dim3                    seedCoordinates,
                                                int                     sphereSize,
                                                float                   outOfPlaneAnisotropy )
{
    ...
    // call our kernel
    InitializeConstraintVolumeKernel<<< gridDimensions, threadBlockDimensions >>>(
        deviceConstraintData,
        volumeDimensions,
        logicalNumThreadBlocksX,
        invNumThreadBlocksX,
                                                   ...
                                                   float  outOfPlaneAnisotropy )
{
    ...
    if ( distance < sphereSize && elementCoordinates.z == seedRegion.z )
    {
        float oldConstraintValue = deviceConstraintData[ arrayIndex ];
        float newConstraintValue = Clamp( ( int )( oldConstraintValue - 127 ), -127, 127 );
        
        deviceConstraintData[ arrayIndexUntiled ] = newConstraintValue;
    }
}
