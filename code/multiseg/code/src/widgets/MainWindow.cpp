...
void MainWindow::volumeDirectoryDescriptionOK()
{
    ...
    // file names
    mVolumeFileDesc.leftFileName  = mLeftFileName;
    mVolumeFileDesc.rightFileName = mRightFileName;
...
{
    QString fileName = QFileDialog::getOpenFileName( this, "Open MultiSeg Project", "projects", "MultiSeg Project Files (*.mproj)" );
...
{
    QString fileName = QFileDialog::getExistingDirectory( this, "Select the directory containing the first DICOM image stack", mVolumeFileDesc.leftFileName.ToQString() );

    if ( fileName != "" )
    {
        mLeftFileName = fileName;
        fileName = QFileDialog::getExistingDirectory( this, "Select the directory containing the second DICOM image stack", mVolumeFileDesc.leftFileName.ToQString() );
...
{
    QString fileName = QFileDialog::getOpenFileName( this, "Open the first raw data file", mVolumeFileDesc.leftFileName.ToQString(), "Raw Data File (*.*)" );

    if ( fileName != "" )
    {
        mLeftFileName = fileName;
        fileName = QFileDialog::getOpenFileName( this, "Open the second raw data file", mVolumeFileDesc.leftFileName.ToQString(), "Raw Data File (*.*)" );
...
{
    boost::filesystem::path filePath( mVolumeFileDesc.leftFileName.ToStdString() );
...
bool MainWindow::saveProjectAs( bool )
{
    ...
    QString projectFileName = QFileDialog::getSaveFileName( this, "Save Project As", "projects", "MultiSeg Project Files (*.mproj)" );

    if ( projectFileName != "" )
    {
        if ( !projectFileName.endsWith( ".mproj", Qt::CaseInsensitive ) )
        {
            projectFileName.append( ".mproj" );
        }

        QDomDocument projectDomDocument;

        QDomElement rootElement    = projectDomDocument.createElement( "Root" );
        QDomElement projectElement = projectDomDocument.createElement( "MultiSegProject" );

        QDir current = QDir::current();

        QString relativeLeftDataPath   = current.relativeFilePath( mVolumeFileDesc.leftFileName.ToQString() );
        QString relativeRightDataPath  = current.relativeFilePath( mVolumeFileDesc.rightFileName.ToQString() );
        QString relativeParametersPath = current.relativeFilePath( currentParametersFile.ToQString() );

        projectElement.setAttribute( "leftDataFile",     relativeLeftDataPath );
        projectElement.setAttribute( "rightDataFile",    relativeRightDataPath );
...
{
    QString fileName = QFileDialog::getExistingDirectory( this, "Select the directory to save the segmentation as a DICOM stack", mVolumeFileDesc.leftFileName.ToQString() );
...
void MainWindow::loadVolume( const VolumeFileDesc& volumeFileDesc, const QString& parameterFileName )
{
    ...
    QFileInfo fileInfo( volumeFileDesc.leftFileName.ToQString() );
...
void MainWindow::openProject( const QString& projectFileName )
{
    ...
    Assert( projectElement.tagName() == "MultiSegProject" );

    VolumeFileDesc volumeFileDesc;

    volumeFileDesc.leftFileName     = projectElement.attribute( "leftDataFile" );
    volumeFileDesc.rightFileName    = projectElement.attribute( "rightDataFile" );
...
{
    int response = QMessageBox::question( this, "MultiSeg", "Are you sure you want to exit?", QMessageBox::Yes, QMessageBox::No );
